...
//
function toBytes(x, bytes) {
	var i, r = new Array(bytes);
	for (i = 0; i < bytes; i++) {
		r[i] = x & 0xFF;
		x = x >> 8;
	}
...
function snowflake(time, mac, seq) {
	var final = new Array(8);
	time = toBytes(time, 5);
	seq = toBytes(seq, 2);
	// 12 bits of sequence (least sig digits)
	// 10 bits of machine (most sig digits, 2 bytes & 0x)
	final[0] = seq[0];
	final[1] = seq[1] & 0x0f; // cut off most significant 4 bytes
	final[1] |= mac & 0x03; // Only the last two digits off mac
	final[2] = (mac & 0x3fc) >> 2; // Next 8 bits of mac
	final[3] = 
	[].concat()
		toBytes(mac, 4),
	)
